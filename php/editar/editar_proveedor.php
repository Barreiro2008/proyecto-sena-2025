...
$stmt_proveedor = $pdo->prepare("SELECT nombre, contacto, telefono FROM proveedores WHERE id = :id");
$stmt_proveedor->bindParam(':id', $proveedor_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($contacto) || empty($telefono)) {
        ...
        try {
            $stmt_update = $pdo->prepare("UPDATE proveedores SET nombre = :nombre, contacto = :contacto, telefono = :telefono WHERE id = :id");
            $stmt_update->bindParam(':nombre', $nombre);
            $stmt_update->bindParam(':contacto', $contacto);
            $stmt_update->bindParam(':telefono', $telefono);
            $stmt_update->bindParam(':id', $proveedor_id);
            $stmt_update->execute();
            ...
            $error_message = "Error al actualizar el proveedor: " . $e->getMessage();
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Editar Proveedor | Variedades Juanmarc</title>
...
<body class="jm-body">
    ...
    <div class="jm-main">
        <div class="jm-navbar">
            <h2><i class="fas fa-edit mr-3"></i>Editar Proveedor</h2>
        ...
        <div class="jm-formulario-container">
            <div class="jm-formulario">
                <div class="jm-formulario-header">
                    ...
                    <p class="jm-formulario-subtitle">ID: #<?php echo htmlspecialchars($proveedor_id); ?></p>
                </div>
